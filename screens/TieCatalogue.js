...
export default class TieCatalogue extends React.Component{

    constructor(props){
        super(props);
        this.state={
            name:this.props.navigation.getParam("details2")['name'],
            image:this.props.navigation.getParam("details2")['avatar_url'],
            subtitle:this.props.navigation.getParam('details2')['subtitle'],
            rate:0,
            count:0,
            size:'42',
            ...
            image1:[
                {url:this.props.navigation.getParam("details2")['avatar_url']}
            ],
            image2:[
    ...
    rate = () => {
        if(this.state.subtitle === "Rent Rs: 20/day"){
            this.setState({
                rate:20
            })
        }else{
    ...
    deletedata = async() => {
        ...
        if(PantInfo.filter(item => item.TieName === this.state.name && item.TieSize === this.state.size).length === 0){
            db.collection('Cart').doc(this.state.userID).collection('Tie').add({
                "TieName":this.state.name,
                "TieQuantity":this.state.count,
                "TieSize":this.state.size,
                "Rate":this.state.rate,
                "userID":this.state.userID,
                "Price":this.state.rate * this.state.count
            })
        }else{
            db.collection('Cart').doc(this.state.userID).collection('Tie').where("TieName","==",this.state.name).get()
            .then(snapshots => {
                ...
                db.collection('Cart').doc(this.state.userID).collection('Tie').doc(this.state.docid).delete()
                .then(()=>{
                    db.collection('Cart').doc(this.state.userID).collection('Tie').add({
                        "TieName":this.state.name,
                        "TieQuantity":this.state.count,
    ...
    render(){
        ...
        return(
            <View style={styles.container}>
                ...
                {this.magnify2()}
                {this.magnify3()}
              <View style={{marginTop:80,width:370}}>
