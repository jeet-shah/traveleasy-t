...
export default class SportCatalogue extends React.Component{
    ...
    constructor(props){
        ...
        this.state={
            ...
            image1:[
                {url:this.props.navigation.getParam("details3")['avatar_url']}
            ],
            image2:[
    ...
    deletedata = async() => {
        ...
        if(PantInfo.filter(item => item.SportName === this.state.name && item.SportSize === this.state.size).length === 0){
            ...
            db.collection('Cart').doc(this.state.userID).collection('Sport').where("SportName","==",this.state.name).get()
            .then(snapshots => {
                ...
                db.collection('Cart').doc(this.state.userID).collection('Sport').doc(this.state.docid).delete()
                .then(()=>{
                    db.collection('Cart').doc(this.state.userID).collection('Sport').add({
                        "SportName":this.state.name,
                        "SportQuantity":this.state.count,
